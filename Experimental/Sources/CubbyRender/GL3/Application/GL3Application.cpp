...
//! In headless server, register window callback function cause segmentation
//! fault.
...
void GL3Application::onWindowResized(GLFWwindow* glfwWindow, int width,
                                     int height)
{
    UNUSED_VARIABLE(glfwWindow);
    if (gApplicationInstance == nullptr ||
        gApplicationInstance->validateApplication() == false)
        std::abort();
...
void GL3Application::onKey(GLFWwindow* glfwWindow, int key, int scancode,
                           int action, int mods)
{
    UNUSED_VARIABLE(glfwWindow);
    if (gApplicationInstance == nullptr ||
...
void GL3Application::onMouseButton(GLFWwindow* glfwWindow, int button,
                                   int action, int mods)
{
    UNUSED_VARIABLE(glfwWindow);
    if (gApplicationInstance == nullptr ||
...
void GL3Application::onMouseCursorPos(GLFWwindow* glfwWindow, double x,
                                      double y)
{
    UNUSED_VARIABLE(glfwWindow);
    if (gApplicationInstance == nullptr ||
...
void GL3Application::onMouseScroll(GLFWwindow* glfwWindow, double deltaX,
                                   double deltaY)
{
    UNUSED_VARIABLE(glfwWindow);
    if (gApplicationInstance == nullptr ||
