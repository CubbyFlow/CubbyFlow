...
class InitializeBuffersAndComputeForces
{
 public:
    ...
    inline CUBBYFLOW_CUDA_DEVICE void operator()(Index i)
    {
        // Initialize buffers
        m_pressures[i] = 0.0f;
        m_pressureForces[i] = make_float2(0, 0);
        ...
        float2 x_i = m_positions[i];
        float2 v_i = m_velocities[i];
 ...
 private:
    ...
    float2 m_gravity;
    float m_viscosity;
    CUDASPHSpikyKernel2 m_spikyKernel;
    const uint32_t* m_neighborStarts;
    const uint32_t* m_neighborEnds;
    const uint32_t* m_neighborLists;
    const float2* m_positions;
    const float2* m_velocities;
    float2* m_smoothedVelocities;
    float2* m_forces;
    const float* m_densities;
    float* m_pressures;
    float2* m_pressureForces;
...
class TimeIntegration
{
 public:
    ...
    {
        float2 x = m_positions[i];
        float2 v = m_velocities[i];
        float2 s = m_smoothedVelocities[i];
        float2 f = m_forces[i];
        float2 pf = m_pressureForces[i];
 ...
 private:
    ...
    float2* m_positions;
    float2* m_velocities;
    float2* m_newPositions;
    float2* m_newVelocities;
    float2* m_smoothedVelocities;
    float2* m_forces;
    float2* m_pressureForces;
};
