...
NearestArraySampler<T, R, 3>::NearestArraySampler(
    ConstArrayAccessor3<T> accessor, const Vector3<R>& gridSpacing,
...
template <typename T, typename R>
NearestArraySampler<T, R, 3>& NearestArraySampler<T, R, 3>::operator=(
    const NearestArraySampler& other)
{
    m_gridSpacing = other.m_gridSpacing;
    m_origin = other.m_origin;
...
template <typename T, typename R>
void NearestArraySampler<T, R, 3>::GetCoordinate(const Vector3<R>& pt,
...
template <typename T, typename R>
std::function<T(const Vector3<R>&)> NearestArraySampler<T, R, 3>::Functor()
    const
{
...
template <typename T, typename R>
LinearArraySampler<T, R, 3>::LinearArraySampler(ConstArrayAccessor3<T> accessor,
...
template <typename T, typename R>
LinearArraySampler<T, R, 3>& LinearArraySampler<T, R, 3>::operator=(
...
T LinearArraySampler<T, R, 3>::operator()(const Vector3<R>& pt) const
{
    ...
    return TriLerp(
        m_accessor(i, j, k), m_accessor(ip1, j, k), m_accessor(i, jp1, k),
        m_accessor(ip1, jp1, k), m_accessor(i, j, kp1), m_accessor(ip1, j, kp1),
        m_accessor(i, jp1, kp1), m_accessor(ip1, jp1, kp1), fx, fy, fz);
}
    ...
    std::array<Vector3<R>, 8>* weights) const
{
    ...
    (*weights)[0] = Vector3<R>(-m_invGridSpacing.x * (1 - fy) * (1 - fz),
                               -m_invGridSpacing.y * (1 - fx) * (1 - fz),
                               -m_invGridSpacing.z * (1 - fx) * (1 - fy));
    (*weights)[1] = Vector3<R>(m_invGridSpacing.x * (1 - fy) * (1 - fz),
                               fx * (-m_invGridSpacing.y) * (1 - fz),
                               fx * (1 - fy) * (-m_invGridSpacing.z));
    (*weights)[2] = Vector3<R>((-m_invGridSpacing.x) * fy * (1 - fz),
                               (1 - fx) * m_invGridSpacing.y * (1 - fz),
                               (1 - fx) * fy * (-m_invGridSpacing.z));
    (*weights)[3] = Vector3<R>(m_invGridSpacing.x * fy * (1 - fz),
                               fx * m_invGridSpacing.y * (1 - fz),
                               fx * fy * (-m_invGridSpacing.z));
    (*weights)[4] = Vector3<R>((-m_invGridSpacing.x) * (1 - fy) * fz,
                               (1 - fx) * (-m_invGridSpacing.y) * fz,
                               (1 - fx) * (1 - fy) * m_invGridSpacing.z);
    (*weights)[5] = Vector3<R>(m_invGridSpacing.x * (1 - fy) * fz,
                               fx * (-m_invGridSpacing.y) * fz,
                               fx * (1 - fy) * m_invGridSpacing.z);
    (*weights)[6] = Vector3<R>((-m_invGridSpacing.x) * fy * fz,
                               (1 - fx) * m_invGridSpacing.y * fz,
                               (1 - fx) * fy * m_invGridSpacing.z);
    (*weights)[7] =
        Vector3<R>(m_invGridSpacing.x * fy * fz, fx * m_invGridSpacing.y * fz,
                   fx * fy * m_invGridSpacing.z);
}
...
template <typename T, typename R>
CubicArraySampler<T, R, 3>::CubicArraySampler(ConstArrayAccessor3<T> accessor,
                                              const Vector3<R>& gridSpacing,
                                              const Vector3<R>& gridOrigin)
    : m_gridSpacing(gridSpacing),
      m_origin(gridOrigin),
...
template <typename T, typename R>
CubicArraySampler<T, R, 3>& CubicArraySampler<T, R, 3>::operator=(
...
T CubicArraySampler<T, R, 3>::operator()(const Vector3<R>& pt) const
{
    ...
    const ssize_t js[4] = { std::max(j - 1, ZERO_SSIZE), j,
                            std::min(j + 1, jSize - 1),
                            std::min(j + 2, jSize - 1) };
    const ssize_t ks[4] = { std::max(k - 1, ZERO_SSIZE), k,
                            std::min(k + 1, kSize - 1),
                            std::min(k + 2, kSize - 1) };
    ...
    for (int kk = 0; kk < 4; ++kk)
    {
        ...
        {
            jValues[jj] =
                MonotonicCatmullRom(m_accessor(is[0], js[jj], ks[kk]),
                                    m_accessor(is[1], js[jj], ks[kk]),
                                    m_accessor(is[2], js[jj], ks[kk]),
                                    m_accessor(is[3], js[jj], ks[kk]), fx);
        }
