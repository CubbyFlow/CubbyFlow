...
CUBBYFLOW_BEGIN_TEST_F(LevelSetSolver2, Reinitialize)
{
	...
	{
		Vector2D ret;
		ret.x = 2.0 * Square(std::sin(PI_DOUBLE * pt.x)) * std::sin(PI_DOUBLE * pt.y) * std::cos(PI_DOUBLE * pt.y);
		ret.y = -2.0 * Square(std::sin(PI_DOUBLE * pt.y)) * std::sin(PI_DOUBLE * pt.x) * std::cos(PI_DOUBLE * pt.x);
		return ret;
	};
