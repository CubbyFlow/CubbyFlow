...
TEST(PointHashGridSearcher3, Build)
{
    ...
    EXPECT_EQ(Vector3Z(1, 1, 37), searcher.GetBucketIndex(points[0]));
    EXPECT_EQ(Vector3Z(37, 1, 0), searcher.GetBucketIndex(points[1]));
    EXPECT_EQ(Vector3Z(-104, 374, 0), searcher.GetBucketIndex(points[2]));

    EXPECT_EQ(21, searcher.GetHashKeyFromBucketIndex(Vector3Z(1, 1, 37)));
    EXPECT_EQ(5, searcher.GetHashKeyFromBucketIndex(Vector3Z(37, 1, 0)));
    EXPECT_EQ(8, searcher.GetHashKeyFromBucketIndex(Vector3Z(-104, 374, 0)));
}