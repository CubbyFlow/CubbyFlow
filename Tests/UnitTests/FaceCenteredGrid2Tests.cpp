...
TEST(FaceCenteredGrid2, Constructors)
{
	...
	{
		EXPECT_DOUBLE_EQ(5.0, grid2.GetU(i, j));
	});
	grid2.ForEachVIndex([&](size_t i, size_t j)
	{
		EXPECT_DOUBLE_EQ(6.0, grid2.GetV(i, j));
	});
	...
	{
		EXPECT_DOUBLE_EQ(5.0, grid3.GetU(i, j));
	});
	grid3.ForEachVIndex([&](size_t i, size_t j)
	{
		EXPECT_DOUBLE_EQ(6.0, grid3.GetV(i, j));
	});
}
...
{
	{
		...
		faceCenteredGrid->ForEachUIndex([&faceCenteredGrid](size_t i, size_t j)
		{
			EXPECT_DOUBLE_EQ(3.0, faceCenteredGrid->GetU(i, j));
		});
		faceCenteredGrid->ForEachVIndex([&faceCenteredGrid](size_t i, size_t j)
		{
